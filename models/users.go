...
const userPwPepper = "8bbe52d5ae80ecedd25c8822a018b676"
const hmacSecretKey = "bae9b48bba254c7a9526d5daa0453a30"
...
func (us *UserService) ByID(id uint) (*User, error) {
	var user User
	db := us.db.Where("users.id = ?", id)
	err := first(db, &user)
	return &user, err
}

// ByEmail looks up the user with the given email address and
// returns that user.
// If the user is found, we will return a nil error.
// If the user is not found, we will return ErrNotFound.
// If there is another error, we will return an error with
...
func (us *UserService) Update(user *User) error {
	if user.Remember != "" {
		user.RememberHash = us.hmac.Hash(u.Remember)
	}
